 ...
 * 
 */
public abstract class CopyOfAbstractFetch implements DockService {
	private static Logger S_Logger = Logger.getLogger(CopyOfAbstractFetch.class);
	...
	protected HttpReturn httpReturn = new HttpReturn();// 公用的返回信息
	...
	protected String execute(String wwwUrl, String encoding) throws Exception {
		...
		try {
			...
			if (S_Logger.isDebugEnabled()) {
				S_Logger.debug("fetch - [com.kingsoft.business.implement.fetch.AbstractFetch] fetch error " + e.getMessage());
			}
		} finally {
			if (response != null) {
				response.close();
